...
GO
/****** Object:  StoredProcedure [dbo].[usp_Attack]    Script Date: 6/27/2017 1:26:56 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	...
	IF(dbo.fn_IsHost(@userId) = 0 AND @nextTurn = 4 OR dbo.fn_IsHost(@userId) = 1 AND @nextTurn = 3 OR @nextTurn = 2)
	BEGIN
		IF(@hitShip = 1)
		BEGIN
			UPDATE GameBoards
